...
use crate::{error::{TalosError, TalosResult}, render::Glyph};
mod win_1252;
...
pub fn validate_page(page: &Page) -> TalosResult<()> {
    if page.len() != 256 {
        return Err(TalosError::InvalidArgument(format!("Page must have 256 entries, got {}", page.len())));
    }

    for (id, &glyph) in page.iter().enumerate() {
        if glyph.len() != 1 {
            return Err(TalosError::InvalidArgument(format!("Page entry '{}' must be a single unicode character, got '{}'", id, glyph)));
        }
    }
    Ok(())
}
