...
pub fn poll_input_into_events(std_in: &mut Stdin, max_poll_input_buffer: u16) -> TalosResult<Option<Vec<Event>>> {
    let bytes = poll_input_bytes(std_in, max_poll_input_buffer)?;
    if let Some(bytes) = bytes {
        parse_byte_stream(bytes)
    } else {
        Ok(None)
    }
}

fn poll_input_bytes(std_in: &mut Stdin, max_poll_input_buffer: u16) -> TalosResult<Option<Vec<u8>>> {
