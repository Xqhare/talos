...
fn main() -> Result<(), talos::TalosError> {
    ...
    while running {
        ...
        let big_area_left = Rect::new(1, 1, ctx.canvas.max_width() / 2, ctx.canvas.max_height());

        let big_area_right = Rect::new(
            ctx.canvas.max_width() / 2 + 1,
            1,
            ctx.canvas.max_width() / 2,
            ctx.canvas.max_height(),
        );
        ...
        let mut large_block_left: Block = Block::new().title("", ctx.codex, false).with_bg_fill();

        large_block_left.style(style_left);

        let mut large_block_right: Block = Block::new().title("", ctx.codex, false).with_bg_fill();

        large_block_right.style(style_right);

        large_block_left.render(&mut ctx, big_area_left);
        ...
        Donec tempor consectetur tortor, ut vestibulum tortor rhoncus vel. In hac habitasse platea dictumst. Curabitur ut diam molestie, placerat massa accumsan, consectetur neque.\n Etiam accumsan hendrerit tellus, et dapibus nunc aliquam posuere. Duis ac nibh libero. Nunc sapien urna, luctus a augue vel, efficitur hendrerit ligula.\n Donec et maximus nunc, a ultrices augue. Quisque viverra purus nisi, dictum congue mi aliquet sit amet. Fusce eu dignissim elit.\n Etiam urna massa, hendrerit vel commodo non, dictum at augue. Pellentesque dapibus diam ut rutrum mattis. Quisque eu mi arcu. Vestibulum bibendum leo et leo pellentesque, vel finibus lectus ornare. Aenean tincidunt facilisis ipsum, a mattis nisl accumsan efficitur.", ctx.codex)
            .align_center();

        left_text.style(left_text_style);

        left_text.render(&mut ctx, left_block_inner);

        large_block_right.render(&mut ctx, big_area_right);
        ...
        let mut block: Block = Block::new()
            .title("Center Block Title", ctx.codex, true)
